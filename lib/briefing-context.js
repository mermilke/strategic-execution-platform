...
// Shared mailbox the briefing reads upcoming 1:1s from (the leader running them).
const CALENDAR_MAILBOX = process.env.CEO_CALENDAR_EMAIL

/** YYYY-MM-DD of the Monday before `weekStart`. */
export function previousMonday(weekStart) {
...
// Fetch the leader's upcoming 14-day calendar. Returns [] if the integration
...
async function fetchCEOCalendar(admin, daysAhead = 14) {
  ...
  try {
    ...
    async function tryGraph(accessToken) {
...
// The clock the briefing is written in (the leader's local timezone).
const APP_TIMEZONE = process.env.APP_TIMEZONE || 'UTC'

/** "Thu, Jun 4 at 10:00 AM CDT" in the app timezone. null if missing. */
function fmtMeetingLabel(iso) {
  if (!iso) return null
...
//   }
function findMeetingsForDR(events, drFullName) {
...
export async function buildBriefingContext(weekStart) {
  ...
  const [
    ...
    fetchCEOCalendar(admin, 14),
  ])

  // Build per-DR rollup
  const drIds = new Set((drs || []).map(d => d.id))
  const objByOwner = new Map()
