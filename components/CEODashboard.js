            ...
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'start', justifyContent: 'space-between', gap: 16 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#1E293B' }}>
                  {historyModal.type === 'objective' ? '🎯' : '📌'} {historyModal.title}
                </div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{historyModal.userName}</div>
              </div>
              <button onClick={() => setHistoryModal(null)} style={{ color: 'var(--text-muted)', cursor: 'pointer', background: 'none', border: 'none', fontSize: 18, lineHeight: 1, padding: 4 }}>✕</button>
            </div>

            <div style={{ overflowY: 'auto', padding: '16px 24px 24px' }}>
              {historyModal.type === 'objective' && historyModal.subs.length > 1 && (
                <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                  <button
                    onClick={() => setExpandedModalSubs(new Set(historyModal.subs.map(s => s.id)))}
                    className="text-xs px-3 py-1.5 rounded-lg"
                    style={{ background: 'rgba(37, 99, 235,0.1)', color: '#2563EB', border: '1px solid rgba(37, 99, 235,0.25)', cursor: 'pointer' }}
                  >Expand all</button>
                  <button
                    onClick={() => setExpandedModalSubs(new Set())}
                    className="text-xs px-3 py-1.5 rounded-lg"
              ...
              {historyModal.subs.map((sub, si) => {
                const isObjectiveView = historyModal.type === 'objective' && historyModal.subs.length > 1
                const isExpanded = !isObjectiveView || expandedModalSubs.has(sub.id)
                ...
                return (
                  <div key={sub.id} style={{ marginBottom: si < historyModal.subs.length - 1 ? (isObjectiveView ? 8 : 0) : 0 }}>
                    {isObjectiveView ? (
                      <div
                        onClick={() => {
                          const next = new Set(expandedModalSubs)
                          next.has(sub.id) ? next.delete(sub.id) : next.add(sub.id)
                          setExpandedModalSubs(next)
                        }}
                        style={{
                    ...
                    {isExpanded && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: isObjectiveView && si < historyModal.subs.length - 1 ? 8 : 0 }}>
