...
function BriefingContent({ briefing, isStreaming }: { briefing: any; isStreaming: boolean }) {
    ...
    <>
      ...
      {briefing.top_items?.length > 0 && (
        <Section label="Top things to know">
          <ol style={topList}>
            {briefing.top_items.map((item: string, i: number) => (
      ...
      {briefing.risks?.length > 0 && (
        <Section label="Risks & blockers">
          {briefing.risks.map((r: any, i: number) => (
      ...
      {briefing.momentum?.length > 0 && (
        <Section label="Momentum">
          {briefing.momentum.map((m: any, i: number) => (
            <div key={i} style={itemRow}>
        ...
        <Section label="Talking points for upcoming 1:1s">
          {briefing.talking_points.map((tp: any, i: number) => (
            <div key={i} style={drCard}>
              ...
              <ul style={drPoints}>
                {tp.points?.map((p: string, j: number) => (
      ...
      {briefing.data_caveats?.length > 0 && (
        <div style={caveats}>
          ...
          <ul style={caveatsList}>
            {briefing.data_caveats.map((c: string, i: number) => (
...
function DevPanel({ meta }: { meta: any }) {
...
function renderInline(raw: unknown) {
